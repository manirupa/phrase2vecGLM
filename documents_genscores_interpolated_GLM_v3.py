...
1) The top 1 nearest word-embedding to each of the query terms picked from the top K ranked set of 
   ...
   Concept selection done for the unigram query and phrasal query and merged into single list
2) The top 1 or 2 tfidf terms associated with each of the top K ranked set of relevant docs 
   Concept selection is merged list from doing the above for unigram query and phrasal query

These are chosen as terms for query expansion, which are essentially the discovered concept 
for the original query doc.
...
def do_this():
    ...
    phrasal_queries = []
    phrasal_querieslines = open(phrasal_query_file).readlines()
    for q in phrasal_querieslines:
        phrasal_queries.append(eval(q))
    ...
    for j in range(len(unigram_queries)):   
        ...
        word2vec_disc_concepts_phr = [item[1] for item in best_matches.values()]
        word2vec_disc_concepts_uni = [item[1] for item in best_matches_uni.values()]
        word2vec_disc_concepts = word2vec_disc_concepts_uni + word2vec_disc_concepts_phr
        tfidf_disc_concepts_phr = tfidf_concepts(matching_docs,phr_topnlines)
        tfidf_disc_concepts_uni = tfidf_concepts(matching_docs,uni_topnlines)
        tfidf_disc_concepts = tfidf_disc_concepts_uni + tfidf_disc_concepts_phr
        concept_tuple = (pquery_docid,pquery,word2vec_disc_concepts,tfidf_disc_concepts,matching_docs,best_matches,best_matches_uni)
